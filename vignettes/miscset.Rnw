...
Currently there is no example available in the vignette due to CRAN error check and \texttt{ggplot2} library incompatibilites. You can try this code to see the results:

<<label=fig_ggplotlist, eval=TRUE, fig.width=6, fig.height=3, fig.align='center'>>=
library(ggplot2)
